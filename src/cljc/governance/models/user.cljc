  ...
  {:spec
   ...
   [{:name     ::first_name
    ...
    {:name     ::admin
     ...
     :component/type :input/boolean
     :required false}
    {:name     ::last_login
     :spec     '(s/nilable ::generic/timestamp)
     :component/type :input/datetime
     :required false}
    {:name ::is_active
     :component/type :input/boolean
     :spec 'boolean?}]
...
(comment
  (clojure.pprint/pprint (-> '(shared/create-model-clj config)
                             macroexpand))
  (-> '(shared/create-model-clj config)
      macroexpand-1)
  ...
  (toucan.db/insert! Users
                     {:email     "someguy@somewhere.com"
                      :is_active true})
  (toucan.db/update! Users (java.util.UUID/fromString "9809ccf6-14cf-461f-aab5-854ea1fd2e82")
                     {:email     "something@somewhere.com"
                                   :is_active false})
